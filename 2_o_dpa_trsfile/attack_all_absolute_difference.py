...
class AESAttack:
    ...
    def read_trs(self, filename):
        ...
        self.n_s_c_p = int((self.n_s * (self.n_s - 1)) / 2)  # The number of samples in a centered abs_diff trace
    ...
    def abs_diff_combining_trace(self, trace, mean_sam):
        """ This function returns a trace that is centered and abs_diff (n_s * (n_s - 1)/2_vector)"""
    ...
    def comb_traces(self):
        """ This function returns all traces from centred abs_diff combining function
        ...
        for i in range(self.n_t):
            combined_traces[i] = self.abs_diff_combining_trace(traces[i], mean_sam)
